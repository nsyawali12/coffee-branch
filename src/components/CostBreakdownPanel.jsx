...
export default function CostBreakdownPanel({ employees, operationalCost, showSalary, darkMode }) {
  ...
  if (!showSalary) {
    return (
      <section className="rounded-xl bg-white dark:bg-slate-800 shadow-sm ring-1 ring-slate-200 dark:ring-slate-700 p-8 text-center text-slate-400">
        🔒 Cost Breakdown Panel contains payroll &amp; rent data — click <span className="font-medium text-slate-500 dark:text-slate-300">&quot;Show Salary Data&quot;</span> in the header to view it.
      </section>
    )
  }
  ...
  return (
    <section className="flex flex-col gap-4">
      <div className="rounded-xl bg-white dark:bg-slate-800 shadow-sm ring-1 ring-slate-200 dark:ring-slate-700 p-4">
        <h2 className="text-sm font-semibold text-slate-700 dark:text-slate-200 mb-3">Cost Breakdown — Total Monthly Operational Cost</h2>
        <div className="flex flex-wrap items-end gap-6">
          <div>
            <p className="text-2xl font-semibold text-slate-800 dark:text-slate-100">{formatUSD(totalOperationalCost)}</p>
            <p className="text-xs text-slate-400 dark:text-slate-500">rent + total payroll</p>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        />
        <PayrollByDepartmentChart employees={employees} darkMode={darkMode} />
        <HrOutsourcingPanel
          hrOutsourcing={operationalCost.hr_outsourcing}
          flcQuote={flcQuote}
          setFlcQuote={setFlcQuote}
        />
        <WorkModeDistributionChart employees={employees} darkMode={darkMode} />
      </div>
    </section>
  )
}
