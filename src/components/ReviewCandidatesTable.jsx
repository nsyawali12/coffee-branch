...
export default function ReviewCandidatesTable({ data }) {
  const candidates = reviewCandidates(data).sort((a, b) => a.vs_average_pct - b.vs_average_pct)
  ...
  return (
    <section className="rounded-xl bg-white dark:bg-slate-800 shadow-sm ring-1 ring-slate-200 dark:ring-slate-700 overflow-hidden">
      <div className="px-4 py-3 border-b border-slate-100 dark:border-slate-700">
        ...
        <p className="text-xs text-slate-400">Internal employees with revenue below 50% of the average</p>
      </div>
        ...
        <table className="min-w-full text-sm">
          <thead className="bg-slate-50 dark:bg-slate-900 text-slate-500 dark:text-slate-400">
            <tr>
              ...
              <th className="px-3 py-2 text-left font-medium">Active Months</th>
          ...
          <tbody className="divide-y divide-slate-100 dark:divide-slate-700">
            {candidates.map((m) => (
              <tr key={m.id} className="hover:bg-slate-50 dark:hover:bg-slate-700/50">
                ...
                <td className="px-3 py-2 whitespace-nowrap text-slate-700 dark:text-slate-300">{m.active_months}</td>
      ...
      <p className="px-4 py-3 text-xs text-slate-400 dark:text-slate-500 border-t border-slate-100 dark:border-slate-700">
        Independent contractors (ИП) excluded from review. Data period: Jan–Jun 2026. Single active month
        insufficient for termination decision — recommend 3-month minimum before action.
      </p>
    </section>
  )
}
